...
:top_calories
	s:^ (.*):<MAX>\1#result_mp_tc1<XAM>:p
	h
	b max_pos
	:result_mp_tc1
		s:^<MAX>([0-9]+)#[^<]+<XAM>$:\1:p
	G
	s:^([0-9]+)\n(.*[> ])\1([ #].*)#[^<]+(<XAM>):\2\3#result_mp_tc2\4\1:
	s:  : :p
	h
	b max_pos
	:result_mp_tc2
		s:^<MAX>([0-9]+)#[^<]+<XAM>:\1 :p
	G
	s:^([0-9]+)([^\n]+)\n(.*[> ])\1([ #].*)#[^<]+(<XAM>)[0-9]+$:\3\4#result_mp_tc3\5\1\2:
	s:  : :p
	b max_pos
	:result_mp_tc3
		s:^<MAX>([0-9]+)#[^<]+<XAM>:\1 :p
	s:.*:<ADD>&#result_ap_tc<DDA>:p
	b add_pos
	:result_ap_tc
		s:^<ADD>([0-9]+)#[^<]+<DDA>$:\1:p
b EOS
...
:user_redirects
	/##result_ap_gc<DDA>/b result_ap_cc
	/##result_mp_mc<XAM>/b result_mp_mc
	/##result_mp_tc1<XAM>/b result_mp_tc1
	/##result_mp_tc2<XAM>/b result_mp_tc2
	/##result_mp_tc3<XAM>/b result_mp_tc3
	/##result_ap_tc<DDA>/b result_ap_tc
b EOS
